#ifndef G2O_BASE_FIXED_SIZED_EDGE_EQ_WITH_LAGRANGIAN_H
#define G2O_BASE_FIXED_SIZED_EDGE_EQ_WITH_LAGRANGIAN_H
...
class G2O_CORE_API BaseFixedSizedEdgeEq : public BaseFixedSizedEdge<2 * D, E, VertexTypes..., VertexLagrangeMultiplier<D>> {
public:
    using VoidEdgeFuncType = std::function<void(BaseFixedSizedEdgeEq<D, E, VertexTypes...>&)>;
    ...
    // Setter
    void setLagrangeMultiplier(const Eigen::Matrix<double, D, 1>& lagrangeMuliplier);
    void setRho(const Eigen::Matrix<double, D, 1>& rho);
    void setConstraintViolationPrev(Eigen::Matrix<double, D, 1>&& input, size_t start = 0);  // in AL method we use the information matrix to save the constraint violation and rho_bar
...
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::initializeJacobians() {
    ...
    auto setZeroHelper = [](auto& jacobian) {
        jacobian.setZero();
    };

    // Apply the lambda to each Jacobian in the tuple
    std::apply([&setZeroHelper](auto&... jacobians) {
        (setZeroHelper(jacobians), ...); // Expand and apply to all tuple elements
    }, this->_jacobianOplus);

    // Set the Jacobian for the Lagrangian vertex
    if constexpr (sizeof...(VertexTypes) > 0) {
        auto& J_vLagrangian = std::get<sizeof...(VertexTypes)>(this->_jacobianOplus);
...
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::computeError() {
    if (this->_vertices.empty() || this->_vertices.size() < sizeof...(VertexTypes) + 1) {
        throw std::runtime_error("Lagrangian vertex missing or invalid index");
    }

    const auto* vLagrangian = static_cast<const VertexLagrangeMultiplier<D>*>(this->_vertices.back());
...
template <int D, typename E, typename... VertexTypes>
int BaseFixedSizedEdgeEq<D, E, VertexTypes...>::getVertexLagrangeMultiplierId() const {
...
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setConstructQuadraticFormImpl(const VoidEdgeFuncType& func) {
...
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::constructQuadraticForm() {
    ...
    if (_constructQuadraticFormImpl) {
        ...
        this->BaseFixedSizedEdge<2 * D, E, VertexTypes..., VertexLagrangeMultiplier<D>>::constructQuadraticForm();
    }
}
...
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setInformation(const InformationType& information) {
...
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setConstraintViolationPrev(Eigen::Matrix<double, D, 1>&& input, size_t start) {
...
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setVertexLagrangeMultiplierId(int id) {
...
// Setter functions
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setLagrangeMultiplier(const Eigen::Matrix<double, D, 1>& lagrangeMultiplier) {
    ...
    auto* vLagrangian = static_cast<VertexLagrangeMultiplier<D>*>(this->_vertices.back());
...
template <int D, typename E, typename... VertexTypes>
void BaseFixedSizedEdgeEq<D, E, VertexTypes...>::setRho(const Eigen::Matrix<double, D, 1>& rho) {
    _rho = rho;
}
...
template <int D, typename E, typename... VertexTypes>
Eigen::Matrix<double, D, 1> BaseFixedSizedEdgeEq<D, E, VertexTypes...>::lagrangeMultiplier() const {
...
template <int D, typename E, typename... VertexTypes>
Eigen::Matrix<double, D, 1> BaseFixedSizedEdgeEq<D, E, VertexTypes...>::rho() const {
    return _rho;
}
...
#endif // G2O_BASE_FIXED_SIZED_EDGE_EQ_WITH_LAGRANGIAN_H
